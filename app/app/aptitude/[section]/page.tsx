...
import { Clock, CheckCircle, XCircle, AlertTriangle, ChevronRight, ChevronLeft, Flag, Loader2 } from 'lucide-react';

/* Fallback hardcoded questions */
const FALLBACK_DATA: Record<string, { question: string; options: string[]; answer: number; explanation: string }[]> = {
  quantitative: [
    ...
    { question: 'If 6 workers can complete a job in 12 days, how many days will 9 workers take?', options: ['8 days', '9 days', '6 days', '10 days'], answer: 0, explanation: '6x12 = 9xd -> d = 8 days.' },
    { question: 'Find the compound interest on 10,000 at 10% per annum for 2 years.', options: ['2,100', '2,000', '1,900', '2,500'], answer: 0, explanation: 'CI = 10000(1.1^2) - 10000 = 2,100.' },
    { question: 'The average of 5 numbers is 18. Four of them are 12, 15, 22, 19. Find the fifth.', options: ['22', '24', '20', '18'], answer: 0, explanation: '5x18 = 90. Sum of 4 = 68. Fifth = 90-68 = 22.' },
    ...
    { question: 'A shopkeeper sells goods at 20% loss on cost price. SP is 160. Find CP.', options: ['200', '180', '192', '220'], answer: 0, explanation: 'CP x 0.8 = 160 -> CP = 200.' },
    ...
    { question: 'A rectangle has length 15 cm and width 8 cm. Find its diagonal.', options: ['17 cm', '15 cm', '20 cm', '13 cm'], answer: 0, explanation: 'sqrt(15^2+8^2) = sqrt(289) = 17 cm.' },
  ],
  logical: [
    { question: 'Complete the series: 2, 6, 12, 20, 30, ?', options: ['42', '40', '36', '44'], answer: 0, explanation: 'Differences: 4,6,8,10,12. Next = 30+12 = 42.' },
    { question: 'If MANGO = 13-1-14-7-15, what is APPLE?', options: ['1-16-16-12-5', '1-15-16-12-5', '1-16-17-12-5', '2-16-16-12-5'], answer: 0, explanation: 'Each letter = its position in alphabet.' },
    { question: 'A is B sister. C is B mother. D is C father. E is D mother. How is A related to D?', options: ['Granddaughter', 'Daughter', 'Grandmother', 'Sister'], answer: 0, explanation: 'D is C father -> B grandfather -> A grandfather. A is D granddaughter.' },
    { question: 'Find the odd one out: 3, 5, 7, 9, 11, 13', options: ['9', '3', '11', '13'], answer: 0, explanation: '9 = 3^2 is a perfect square; all others are prime.' },
    ...
    { question: 'Walking at 3/4 of his usual speed, a person reaches office 20 min late. Usual time?', options: ['60 min', '45 min', '80 min', '50 min'], answer: 0, explanation: 'If speed = 3/4, time = 4/3 normal. Extra = 1/3 x T = 20 -> T = 60 min.' },
    { question: 'If all Roses are Flowers and some Flowers are Red, then which is definitely true?', options: ['Some roses may be red', 'All roses are red', 'No roses are red', 'All red are flowers'], answer: 0, explanation: 'We cannot conclude all roses are red, but some may be.' },
    { question: 'Point A is 30m East of B. C is 20m South of A. D is 30m West of C. What direction is D from B?', options: ['South', 'East', 'North', 'West'], answer: 0, explanation: 'D is directly South of B.' },
    { question: 'Find the next term: 1, 4, 9, 16, 25, ?', options: ['36', '30', '49', '35'], answer: 0, explanation: 'Perfect squares: 6^2 = 36.' },
  ...
  verbal: [
    { question: 'Choose the word most similar in meaning to BENEVOLENT:', options: ['Kind', 'Cruel', 'Selfish', 'Harsh'], answer: 0, explanation: 'Benevolent = kind, generous.' },
    { question: 'Choose the antonym of VERBOSE:', options: ['Concise', 'Wordy', 'Talkative', 'Lengthy'], answer: 0, explanation: 'Verbose = too many words. Antonym = concise.' },
    { question: 'Identify the error: She don not know the answer.', options: ['don -> does not', 'She -> Her', 'know -> knew', 'No error'], answer: 0, explanation: 'Third person singular requires does not.' },
    { question: 'Fill in: The manager along with his teammates ___ present at the meeting.', options: ['was', 'were', 'are', 'have been'], answer: 0, explanation: 'Along with does not change subject. Manager is singular -> was.' },
    { question: 'Select the correctly spelled word:', options: ['Occurrence', 'Occurence', 'Occurance', 'Ocurrence'], answer: 0, explanation: 'Correct spelling is Occurrence.' },
    { question: 'BOOK : LIBRARY :: PAINTING : ?', options: ['Gallery', 'Museum', 'Art', 'Canvas'], answer: 0, explanation: 'Books in Library; Paintings in Gallery.' },
    { question: 'Rearrange: always / should / the truth / you / speak', options: ['You should always speak the truth', 'You always should speak the truth', 'Always you should speak the truth', 'You should speak always the truth'], answer: 0, explanation: 'Correct: You should always speak the truth.' },
    ...
    { question: 'Choose the correct idiom meaning for Bite the bullet:', options: ['Endure pain stoically', 'Shoot someone', 'Eat quickly', 'Face a problem'], answer: 0, explanation: 'Bite the bullet = endure pain bravely.' },
    { question: 'Success is not final, failure is not fatal: it is the courage to continue that counts. This quote emphasizes:', options: ['Perseverance', 'Talent', 'Luck', 'Intelligence'], answer: 0, explanation: 'Emphasizes perseverance.' },
  ],
};
...
function shuffleQuestions(qs: { question: string; options: string[]; answer: number; explanation: string }[]) {
...
export default function QuizPage() {
  ...
  const TOTAL_TIME = section === 'logical' ? 25 * 60 : 20 * 60;
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/generate-aptitude', {
        ...
        body: JSON.stringify({ section, count: 10 }),
      });
      ...
      const fallback = FALLBACK_DATA[section];
      if (fallback) {
        const shuffled = shuffleQuestions(fallback);
    ...
    if (user) {
      fetch('/api/db', {
        ...
        body: JSON.stringify({
          ...
          attempt: { userEmail: user.email, section, score: sc, total: questions.length, accuracy: acc, timeUsed: tu, answers },
        }),
      }).catch(() => {});
    }
      ...
      <main className="main-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <div className="card-no-hover" style={{ maxWidth: 560, width: '100%', padding: 48, textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: 32, lineHeight: 1.6 }}>
            10 questions &bull; {TOTAL_TIME / 60} minutes &bull; No negative marking
          </p>
          ...
          <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10, padding: '14px 18px', marginBottom: 28, textAlign: 'left' }}>
            ...
            <ul style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <li>Fresh AI-generated questions every attempt</li>
          ...
          {loadError && (
            <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '12px 16px', marginBottom: 16, color: '#f87171', fontSize: '0.85rem' }}>
              {loadError}
            </div>
          )}
          <button className="btn-primary" onClick={loadQuestions} disabled={loading} style={{ width: '100%', padding: '16px', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: loading ? 0.7 : 1 }}>
            {loading ? (<><Loader2 size={18} className="spin-icon" /> Generating Questions...</>) : '🚀 Begin Quiz'}
          </button>
